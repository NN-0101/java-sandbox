 ...
 * 处理的消息类型。它起到了“安全网”的作用，确保任何消息都不会被静默丢弃，同时为系统
 * 监控和协议完整性验证提供重要信息。
 *
 * <p><b>核心职责：</b>
 * <ul>
 *   <li><b>未知消息处理：</b>捕获管道中没有处理器能够处理的消息类型，避免消息在管道中
 *       无限传播或被静默丢弃</li>
 *   <li><b>异常监控：</b>记录未处理的消息类型和相关信息，便于发现协议不匹配、遗漏处理器
 *       或客户端异常行为</li>
 *   <li><b>消息终结：</b>作为管道中的最后一个处理器，终止消息的继续传播，避免资源浪费</li>
 *   <li><b>协议完整性验证：</b>通过监控未处理消息，确保所有定义的消息类型都有对应的处理器实现</li>
 * </ul>
 *
 * <p><b>工作流程：</b>
 * <ol>
 *   <li>从 Channel Attribute 中获取设备 MAC 地址（如果设备已认证）</li>
 *   <li>记录错误级别的日志，包含设备标识和未处理的消息类型</li>
 *   <li>可根据业务需要，向设备返回错误响应（目前注释，可根据需求启用）</li>
 * </ol>
 *
 * <p><b>设计理念：</b>
 * <ul>
 *   <li><b>fail-fast（快速失败）：</b>快速发现未处理的消息类型并记录错误，避免消息被静默丢弃，
 *       导致业务逻辑缺失或数据不一致</li>
 *   <li><b>监控告警：</b>通过错误日志触发监控系统告警，及时通知开发人员协议不匹配或遗漏处理器的问题</li>
 *   <li><b>协议完整性：</b>确保所有定义的消息类型都有对应的处理器，维护协议的完整性和系统的稳定性</li>
 ...
 * <p><b>使用场景：</b>
 * <ul>
 *   <li><b>协议版本不匹配：</b>设备端使用了新版本协议，发送了服务端不认识的消-息类型</li>
 *   <li><b>遗漏处理器：</b>新增了消息类型，但忘记添加对应的业务处理器</li>
 ...
 * <p><b>响应建议：</b>
 * <ul>
 *   <li>对于生产环境，建议启用错误响应，告知设备消息类型不支持，帮助客户端快速发现问题</li>
...
public class DeviceDefaultHandler extends SimpleChannelInboundHandler<DeviceMessageBO> {
     ...
     * 处理未匹配的消息类型
     *
     * <p>当消息到达此处理器时，意味着前面所有的业务处理器（如 DeviceConnHandler、
     * DeviceHeardHandler 等）都无法处理该消息类型。此方法负责记录错误日志，
     * 并可选择性地向设备返回错误响应。
     *
     * <p><b>处理步骤：</b>
     * <ol>
     *   <li>从 Channel Attribute 中获取设备 MAC 地址（如果设备已认证）</li>
     *   <li>根据设备是否认证，记录相应的错误日志，包含未处理的消息类型</li>
     *   <li>【可选】构造错误响应并发送给设备，告知消息类型不支持</li>
     * </ol>
     *
     * <p><b>注意事项：</b>
     * <ul>
     ...
     * @param msg 未处理的设备消息对象，包含消息类型和业务数据
     */
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, DeviceMessageBO msg) {
        ...
        if (macId != null) {
            // 已认证设备发送了未知消息类型
            log.error("设备 {} 发送了未处理的消息类型: {}, 消息内容: {}",
                    macId, msg.getType(), msg);
        } else {
            // 未认证设备发送了未知消息类型
            log.error("未认证设备发送了未处理的消息类型: {}, remoteAddress={}, 消息内容: {}",
                    msg.getType(), ctx.channel().remoteAddress(), msg);
        }
        ...
        /*
        try {
            PushDeviceMessageBO errorResponse = new PushDeviceMessageBO();
            errorResponse.setMessageType(MessageTypeEnum.ERROR.getValue());
            errorResponse.setContent("unsupported message type: " + msg.getType());
     ...
     * 默认将异常传递给下一个处理器，但由于这是最后一个处理器，这里选择
     * 记录日志并关闭连接。
     *
