 ...
 *   <li><b>消息路由：</b>作为消息处理管道的入口，将解析后的设备上行消息业务对象通过
 *       {@link ChannelHandlerContext#fireChannelRead(Object)} 传递给下游的业务处理器链，
 *       由各处理器根据 {@link DeviceUpMessageTypeEnum} 进行具体的业务处理</li>
 * </ul>
 *
 * <p><b>工作流程：</b>
 * <ol>
 ...
 *       <li>使用 FastJson2 将 JSON 文本解析为 {@link DeviceUpMessageBO} 业务对象</li>
 *       <li>将解析后的业务对象传递给管道中的下一个处理器（如 {@link DeviceConnHandler}、{@link DeviceHeardHandler} 等）</li>
 *     </ul>
 *   </li>
 ...
 *       <li>如果存在 macId（即设备已认证），从 {@link DeviceChannelGroup} 中移除该设备的连接记录</li>
 ...
 * <p><b>设计理念：</b>
 * <ul>
 *   <li><b>关注点分离：</b>只负责协议转换和连接生命周期管理，设备上行消息的业务逻辑完全交由下游处理器，
 *       符合单一职责原则，便于维护和测试</li>
 ...
 * pipeline.addLast(new DeviceDefaultHandler());   // 5. 兜底处理器 - 处理未知上行消息类型
 * </pre>
 *
 * <p><b>设备上行消息格式示例：</b>
 * <pre>
 * // 设备发送的 JSON 消息（messageType 字段对应 {@link DeviceUpMessageTypeEnum} 中的枚举值）
 * {
 *   "messageType": 101,                // DeviceUpMessageTypeEnum.CONN.getCode()
 *   "macId": "AA:BB:CC:DD:EE:FF",      // 设备 MAC 地址
 *   "timestamp": 1742313600000,         // 时间戳
 *   "data": { ... }                      // 业务数据
 * }
 * </pre>
 *
 * <p><b>平台下行消息示例：</b>
 * <pre>
 * // 服务端返回的下行消息（messageType 字段对应 {@link PlatformDownDeviceMessageTypeEnum} 中的枚举值）
 * {
 *   "messageType": 901,                  // PlatformDownDeviceMessageTypeEnum.CONN_RESPONSE.getCode()
 *   "content": "auth success",            // 响应内容
 *   "timestamp": 1742313600123            // 时间戳
 * }
 * </pre>
 *
 * @author 0101
...
public class DeviceFrameHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
     ...
     *   <li>使用 FastJson2 将 JSON 文本解析为 {@link DeviceUpMessageBO} 业务对象，
     *       该对象的 messageType 字段对应 {@link DeviceUpMessageTypeEnum} 中的枚举值</li>
     ...
     *   <li>此处不做业务验证（如消息类型检查），由下游处理器根据 {@link DeviceUpMessageTypeEnum} 完成</li>
     * </ul>
     *
     * @param ctx Channel 处理器上下文，包含管道、通道、属性等信息
     * @param msg 设备发送的 WebSocket 文本帧，包含 JSON 格式的设备上行消息数据
     */
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, TextWebSocketFrame msg) {
        ...
        // 使用 FastJson2 将 JSON 字符串解析为设备上行消息业务对象
        DeviceUpMessageBO deviceUpMessageBO = JSONObject.parseObject(request, DeviceUpMessageBO.class);
     ...
     * <p><b>设计要点：</b>
     * <ul>
     *   <li>必须与 {@link DeviceConnHandler#process(ChannelHandlerContext, DeviceUpMessageBO)}
     *       成对使用，确保资源被正确释放</li>
     *   <li>如果未调用此方法，已断开的设备连接会一直存在于 {@link DeviceChannelGroup} 中，
     *       导致内存泄漏和后续平台下发消息推送失败</li>
     * </ul>
     *
     * @param ctx Channel 处理器上下文
     */
