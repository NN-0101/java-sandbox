...
import com.sandbox.services.living.model.dto.CaptchaLoginDTO;
import com.sandbox.services.living.model.dto.auth.LoginDTO;
import com.sandbox.services.living.model.vo.auth.LoginVO;
...
public class AuthController {
    ...
    public R<LoginVO> login(@RequestBody LoginDTO loginDTO) {
            ...
            // 创建令牌
            CustomToken token = tokenManager.createToken(userDetails.getUserId(), userDetails.getUsername());
    ...
    public R<?> refresh(@RequestHeader("Authorization") String token) {
        if (token != null && token.startsWith("Bearer ")) {
            String tokenValue = token.substring(7);
            CustomToken newToken = tokenManager.refreshToken(tokenValue);
